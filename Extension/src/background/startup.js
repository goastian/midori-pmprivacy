...
import { getMidoriSettings } from '../common/midori-settings';
...
export const startup = async function () {
    async function onLocalStorageLoaded() {
        ...
        application.start({
            async onInstall() {
                try {
                    log.info('Apply default Midori settings..');
                    const midoriSettings = getMidoriSettings({ protectionLevel: 'standard' });

                    const result = await settingsProvider.applySettingsBackup(JSON.stringify(midoriSettings));
                    if (result) {
                        log.info('Midori settings applied successfully');
                    } else {
                        log.error('Error applying Midori settings: unknown', midoriSettings);
                    }
                } catch (e) {
                    log.error('Error applying Midori settings: {0}', e.message);
                }
            },
        });
    }
